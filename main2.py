...
##Filter
factorIN = 0.6  # bestimmt Gewichtung des neuen input-Wertes
factor0 = 0.2   # "         "         der alten Werte
factor1 = 0.07
factor2 = 0.03

##PID
PIDrange = 60 ##Wie viele vergangene Werte zur PID berechnung genommen werden
factorp = 1
factori = 0.01 #Faktor mit welchem die "i"-korrektur angeglichen wird
factord = 0.01 #                        d
factord2 = 0.01  ## für weiter entfernte "d"-messungen
d2range = 5 #Ort der d2 Messung (im Array)

prevISTX = [0]*121  ## Messungen werden hier gespeichert
prevSOLLX = [0]*121 
prevISTY = [0]*121  
prevSOLLY = [0]*121 
...
def motorX(): # controls x-axis
    ...
    while True:
        ...
        x1.ChangeDutyCycle(xmotorposition1[gentarget(output1[1],output2[1],0)])
        x2.ChangeDutyCycle(xmotorposition2[gentarget(output1[1],output2[1],0)])
        x3.ChangeDutyCycle(xmotorposition3[gentarget(output1[1],output2[1],0)])
        time.sleep(DT/2)
...
def motorY(): # controls y-axis
    ...
    while True:
        ...
        #y2.ChangeDutyCycle(ymotorposition1[y])
        #y3.ChangeDutyCycle(ymotorposition1[y])
        y1.ChangeDutyCycle(ymotorposition1[gentarget(output1[0],output2[0],1)])
        y2.ChangeDutyCycle(ymotorposition2[gentarget(output1[0],output2[0],1)])
        y3.ChangeDutyCycle(ymotorposition3[gentarget(output1[0],output2[0],1)])

def gentarget(istwert, sollwert, axis):
    if axis == 0:
        istout = istwert*factorIN + prevISTX[0]*factor0 + prevISTX[12]*factor1 + prevISTX[25]*factor2
        #PID - P
        perrorx = sollwert - istwert
        corrp = perrorx * factorp
        #I
        errorx = 0
        n = 0
        while (n < PIDrange):
            errorx += (prevSOLLX[n] - prevISTX[n])/ 60  ##Summe der Fehler der letzen 1.2 Sekunde
            n += 1
        corri = errox * factori
        #D
        corrd = sollwert - prevISTX[0] ## Veränderungsdifferenz zu 20ms vorher
        corrd2 = sollwert - prevISTX[d2range]  ## Veränderungsdiff. zu 200ms vorher

        output = istout + corrp + corri + corrd * factord + corrd2 * factord2

        del prevISTX[0]
        del prevSOLLX[0]
        prevISTX.append(istwert) #speichert angepassten output im array
        prevSOLLX.append(sollwert)

    elif axis == 1:
        istout = istwert*factorIN + prevISTY[0]*factor0 + prevISTY[12]*factor1 + prevISTY[25]*factor2

        #PID - P
        perrory = sollwert - istwert
        corrp = perrory * factorp
        #I
        errory = 0
        n = 0
        while (n < PIDrange):
            errory += (prevSOLLY[n] - prevISTY[n])/ 60  ##Summe der Fehler der letzen 1.2 Sekunde
            n += 1
        corri = erroy * factori
        #D
        corrd = sollwert - prevISTY[0] ## Veränderungsdifferenz zu 20ms vorher
        corrd2 = sollwert - prevISTY[d2range]  ## Veränderungsdiff. zu 200ms vorher

        output = istout + corrp + corri + corrd * factord + corrd2 * factord2

        del prevISTY[0]
        del prevSOLLY[0]
        prevISTY.append(istwert) #speichert angepassten output im array
        prevSOLLY.append(sollwert)
    return (output)
...
def filter_stabilized(): # combines raw sensor data from stabilized pair
    ...
    while True:
        ...
        if iterations == 8*(1/DT): # set values to zero once values are stabilized based on fixed iteration number (as a function of the master clock)
            ...
            offset[1] = 0.0 - output2[1] # zero yo
            output2[0] = 0 # output to 0 for instant respnse (avoid values in- or decrementing)
